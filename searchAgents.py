    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                successors.append( ( ((nextx, nexty), tuple(visited)), action, cost) )
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    """nonVisited = state[1] #this sotre the corners that have not been visited

    currentState = state[0] #this indicate the current state

    ManDis = []

    heuristic = 0

    if len(nonVistied) == 0: #if there is no more corners to visit return the hrutitic
        return heuristic

    for corners in nonVisited:
        dis"""
    ...
    for corners in nonVisited:
        dist = abs(corners[0] - currentpos[0]) + abs(corners[1] - currentpos[1])
        dis.append((dist,corners))
    dis = sorted(dis)

    heuristic = heuristic + dis[0][0]
    

    closest_corner = dis[0][1]
    #print(dis[0],[0])

    nonVisited = list(nonVisited)
    for corners in nonVisited:
        if corners == closest_corner:
            nonVisited.pop(nonVisited.index(corners))
    ...
    distances = []
    for corners in nonVisited:
        dist = abs(corners[0] - closest_corner[0]) + abs(corners[1] - closest_corner[1])
        distances.append((dist,corners))
